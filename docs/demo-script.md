...
3. `5` USDC-equivalent multi-chain Ika signed intent is approved after Polet policy gating and returns a destination-chain sign-only digest, an unsigned Polet `approve_ika_message_as_session` transaction, and MessageApproval proof metadata.
...
5. `3:05-4:15` Required outcome 3: approved multi-chain Ika signed intent.
   Run `5` USDC-equivalent multi-chain Ika. Show dWallet, MessageApproval, destination-chain message digest, signature scheme, CPI authority, and unsigned Polet approval transaction for the session signer. If live smoke was run, show MessageApproval signature evidence. Say no bridgeless asset settlement is executed.
6. `4:15-4:50` Close.
...
6. Required outcome 1: run the blocked Ika request first.
   ...
   Say: "The agent asked for a multi-chain Ika signed intent above the private limit. Polet blocks it before Ika approval, so there is no dWallet approval data to inspect or misuse."
...
8. Required outcome 3: run the approved multi-chain Ika signed-intent path.
   ...
   Say: "The same confidential guardrail model extends to Ika. Polet approves only the in-limit multi-chain signed-intent request, maps it into a destination-chain sign-only digest, then prepares an unsigned Polet approval transaction that can CPI-call Ika `approve_message`. The frontend does not sign, broadcast, or settle bridgeless assets."
